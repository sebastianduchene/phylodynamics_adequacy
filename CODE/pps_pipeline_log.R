...
-->
  ...
  if(sd(posterior_log_data$growthRate.) > 0){
    growth_rate_lnorm <- c(mean(log(posterior_log_data$growthRate.)), sd(log(posterior_log_data$growthRate.)))
  }else{
...
#  xml_temp <- gsub('GROWTH_RATE_RATE', growth_rate[2], xml_temp)
  ...
  xml_temp <- gsub('GROWTH_RATE_MEAN', growth_rate_lnorm[1], xml_temp)
  xml_temp <- gsub('GROWTH_RATE_SD', growth_rate_lnorm[2], xml_temp)
