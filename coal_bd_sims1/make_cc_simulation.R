...
posterior_params_file <- "cc_veronika_tree.log"
...
for(i in 1:length(posterior_samples)){
      r_xml_temp <- gsub('CONSTANT_POPULATION_SIZE', posterior$popSize[posterior_samples[i]], xml_temp)
      cat(r_xml_temp, file = paste0('cc_get_pps.xml', i), sep = '\n')
      system(paste0('java -jar ~/Desktop/phylo_programs/beast243/lib/beast.jar -overwrite cc_get_pps.xml', i))
