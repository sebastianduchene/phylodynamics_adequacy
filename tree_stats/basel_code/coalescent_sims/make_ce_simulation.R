...
input_tree <- read.tree('vero_dated.tree')


## 
posterior_params_file <- "ce_veronika.log"
...
for(i in 1:length(posterior_samples)){
      xml_temp <- gsub('E_POP_SIZE', posterior$ePopSize[posterior_samples[i]], xml_temp)
      xml_temp <- gsub('GROWTH_RATE', posterior$growthRate[posterior_samples[i]], xml_temp)
      cat(xml_temp, file = 'ce_get_pps.xml', sep = '\n')
      system('java -jar ~/Desktop/phylo_programs/beast243/lib/beast.jar -overwrite ce_get_pps.xml')
