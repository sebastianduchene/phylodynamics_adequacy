...
input_tree <- read.tree('../bd_sim_100.trees')[[1]]
posterior_params_file <- "random_trees_bd_1.log"
...
for(i in 1:length(posterior_samples)){
      xml_temp <- gsub('SAMPLING_PROP_MEAN', posterior$samplingProportion[posterior_samples[i]], xml_temp)
      xml_temp <- gsub('BECOME_UNINFECTIOUS_MEAN', posterior$becomeUninfectiousRate[posterior_samples[i]], xml_temp)
      r0_means <- paste(round(posterior[posterior_samples[i], grep('R0', colnames(posterior))], 2), collapse = ' ')
      xml_temp <- gsub('R0_MEAN', r0_means, xml_temp)
      cat(xml_temp, file = 'bd_get_pps.xml', sep = '\n')
      system('java -jar ~/Desktop/phylo_programs/beast243/lib/beast.jar -overwrite bd_get_pps.xml')
